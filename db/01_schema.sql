...
-- =====================================================================
--
-- ▣ 숫자 타입 선택 — TINYINT / SMALLINT 를 쓰지 않는다
--
--   Hibernate 는 Java 타입에서 JDBC 타입 코드를 정하고, ddl-auto: validate 가
--   그 코드를 실제 컬럼과 대조한다.
--       int / Integer  -> INTEGER  -> INT
--       short / Short  -> SMALLINT
--       byte / Byte    -> TINYINT
--   카운터·일수·연령을 TINYINT/SMALLINT 로 두면 엔티티를 byte/short 로
--   낮춰야 하고, UNSIGNED(0~255) 범위가 Java byte(-128~127)를 넘어
--   조용한 오버플로 위험이 생긴다.
--   저장 공간 차이는 이 규모에서 무의미하다. INT 로 통일한다.
--   (실측: TINYINT UNSIGNED <-> Integer 는 Schema validation 실패)
--
-- ▣ 재실행 안전성
...
CREATE TABLE IF NOT EXISTS icd_code (
  ...
  age_min             INT              NULL   COMMENT 'JPA 매핑이 Integer 다. TINYINT 로 두면 validate 실패',
  age_max             INT              NULL,
